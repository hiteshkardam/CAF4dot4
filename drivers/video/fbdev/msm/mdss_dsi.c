...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	struct mdss_panel_data *pdata;
	printk("LINE 3547: mdss_dsi_ctrl_probe");
	...
	struct device_node *pan_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	printk("FINDING vddio-switch-supply");
	ret = of_property_read_string(supply_node, "regulator-name", &st);
	printk("FINDING regulator-name");
	if (ret) {
		pr_err("%s: error reading name.\n", __func__);
		printk("ERROR READING NAME");
	}
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	 ...
	 */
	struct mdss_panel_data *pdata = &ctrl_pdata->panel_data;
	printk("LINE 4519: mdss_dsi_parse_gpio_params");
	...
	struct device_node *pan_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mipi  = &(pinfo->mipi);
	printk("LINE 4628: dsi_panel_device_register");
	pinfo->type =
